...
function getAllSkills(users) {
    const allSkills = users.flatMap(a => a.skills);
    const uniqueSkills = [...new Set(allSkills)];

    return uniqueSkills;
}

// filtered means users with two friends and with age less than 30
function getTotalBalanceOfFilteredUsers(users) {
    const filteredUsers = users.filter(x => x.age < 30 && x.friends.length === 2)
    const totalBalance = filteredUsers.reduce((prev, cur) => prev + cur.balance, 0);
...
console.log(getTotalBalanceOfFilteredUsers(users));
...
//  a = { 1: 100, 2: 1000 }; b = { 2: 999, 3: 10000 }; 
//  ab = { ...a, ...b }; console.log(ab) // => {1: 100, 2: 999, 3: 10000}

// Rest:
//  it collects remaining elements into a designated variable, 
//  facilitating flexible function definitions and array manipulation.
//  it's very useful to use with functions, like params[] in c#. 
//  example:
//  function func(...args) {
//   return args;
//  }
...
const addUsersToDocument = (users, elem) => {
    ...
    for (const user of users) {
        html += `<li onclick="alert('Hello, ${user.name}')">${user.name}</li>`;
    }
    elem.innerHTML = html;
};
